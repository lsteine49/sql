    ...
    TRIM(
        CASE
            WHEN INSTR(product_name, '-') > 0 THEN
                SUBSTR(product_name, 1, INSTR(product_name, '-') - 1)
        END
    ) AS simple_product_name,
    TRIM(
        CASE
            WHEN INSTR(product_name, '-') > 0 THEN
...
--3)
SELECT market_date, total_sales, 'Best Day' AS best_vs_worst
FROM ranked_sales
WHERE rank_desc = 1

UNION

SELECT market_date, total_sales, 'Worst Day' AS best_vs_worst
FROM ranked_sales
WHERE rank_asc = 1;
...
SELECT
    ...
    vendor_inventory.original_price * 5 * COUNT(customer.customer_id) AS revenue_per_product
